...
let
  ...
  data-jsonschema =
    writeConfig "phishing-url-data-jsonschema.json"
      cell.jsonschemas.phishing-url-data-jsonschema;
in
{
  check = writeShellApplication {
    ...
    text = ''
      check-jsonschema --schemafile ${data-jsonschema} ${
        writeConfig "phishing-url-data.json" cell.config.phishing-url-data
      }
    '';
  };
